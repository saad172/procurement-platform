...
import type { loadProgramPage } from '@/db/queries/program-page';
import { CountryBreakdown, MatchOutcomes, SharedOwnership, SupplierMap } from './charts';
import { RunPanel } from './run-panel';
import { SupplierTable } from './supplier-table';

/**
 * The Program page's sections (SPEC §13.2), one component per `<h2>` — the
 * page itself is only its loader call and this sequence, in order.
 */
type Data = NonNullable<Awaited<ReturnType<typeof loadProgramPage>>>;

/** ── The answers, before any of the apparatus ── */
export function Answers({ data }: { data: Data }) {
  const { answers, running } = data;
  return (
    <>
      {answers.map((answer) => (
...
/**
 * "What you are buying" — the Category ledger, its own component because it
 * is the largest single table on the page: HS lines, bidders and the state of
 * each Category's argued case.
 */
function CategoryLedger({ data, programId }: { data: Data; programId: string }) {
  const { program, biddersByCategory, recByCategory, attemptByCategory } = data;
  return (
    <>
      ...
      */}
      <p className="note" style={{ margin: '0.6rem 0 1.6rem', maxWidth: '56rem' }}>
        Base duty is the ordinary rate for that code into {data.program.importingCountry}. It is a
        floor, not a landed cost — surcharges that key on where a part is actually made are not
        folded in. Open a category for the full picture.
      </p>
    </>
  );
}

/** One row of the Category ledger. */
function CategoryRow({
  ...
  category: Data['program']['categories'][number];
  programId: string;
  bidders: number;
  rec: ReturnType<Data['recByCategory']['get']>;
  attempt: ReturnType<Data['attemptByCategory']['get']>;
}) {
  const line = category.hsLines.find((l) => l.isDefault);
  /*
    `hsLines` holds every candidate classification, not just the scored one.
    The header used to read "Default HS line", and that word Default was the
    only thing saying a choice had been made — dropping the jargon dropped the
    signal with it, so the count says it in words. ENC has three candidates in
    a 0.4-point band; BAT has one and is settled. That difference is worth a
    glance.
  */
  ...
  return (
    <tr>
      ...
      <td>{category.name}</td>
      <td className="num">{bidders}</td>
      <td>
        {rec ? (
          <>
            <Link href={`/program/${programId}/category/${category.id}/recommendation` as never}>
              Version {rec.n}
            </Link>{' '}
            <span className={`badge ${rec.evaluatorOutcome === 'passed' ? 'good' : 'warn'}`}>
              {rec.evaluatorOutcome.replace(/_/g, ' ')}
            </span>
            {rec.awardedTo ? <div className="note">awards {rec.awardedTo}</div> : null}
            {rec.humanMark ? (
              <div className="note">marked {rec.humanMark.replace(/_/g, ' ')} by a person</div>
            ) : null}
          </>
        ) : attemptSays && attempt ? (
          <>
  ...
  return (
    <>
      ...
      */}
      ...
      <CategoryLedger data={data} programId={programId} />
