...
/**
 * One Run page's sections (SPEC §18.4, §18.6) — one component per `<h2>`,
 * plus `Progress` and `PausedOnBudget` for the chrome ahead of the first one.
 *
 * **This is the page a click on Run lands on**, so `Progress` has to answer
 * *is anything happening?* before anything else on the page does.
 */
type Data = NonNullable<Awaited<ReturnType<typeof loadRunPage>>>;

/**
 * The progress strip, and the only thing on the page that moves. It is first
...
/**
 * The only state that returns to `running`. The run budget is a spending
...
/** ── Jobs ── */
export function Jobs({ data, programId, runId }: { data: Data; programId: string; runId: string }) {
  const { jobs, activity, subjects, stuck } = data;
  return (
    <>
      <h2>Jobs</h2>
    ...
    <tr>
      ...
      <td>
        {/*
          ...
          nothing, and this row is also the way back to the page the result
          lands on.
        */}
      ...
      <td className="num note">
        {/*
          ...
          against it. A Job that runs a model has both numbers and they are
          different questions: how much reasoning it did, and how much it
          spent doing it.
        */}
      ...
      <td className="num note">
        {job.tokenCap === 0
          ? 'n/a'
          : `${(activity?.tokens ?? 0).toLocaleString('en-US')} / ${job.tokenCap.toLocaleString('en-US')}`}
      </td>
      <td>
