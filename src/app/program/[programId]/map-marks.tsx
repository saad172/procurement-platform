  ...
  const labelSpot = (
    ...
    for (const degrees of angles) {
      ...
      const clear = obstacles.every(
        (o) => Math.hypot(o.x - x, o.y - y) > o.r + r * 0.9,
      );
  ...
  return (
    <g className="map-marks">
      {clusters.map((cluster) => {
        ...
        const open = spiderfied === cluster.key;
        const cr = clusterRadius(cluster.members.length, r);

        if (open) {
          // Fanned onto a ring, each on a leader line back to the true point.
          // The line is the honesty: drawn here, actually there.
          const spread = cr * 2.4;
          return (
            <g key={cluster.key} className="map-spider">
        ...
        return (
          <g
            ...
            onClick={() => {
              onSelect(null);
              if (atZoomCap) setSpiderfied(cluster.key);
          ...
          >
            ...
            */}
            {(() => {
              const spot = labelSpot(cluster.x, cluster.y, cr, [
                ...
                ...clusters
                  ...
                  .map((other) => ({
                    x: other.x,
                    y: other.y,
                    r: clusterRadius(other.members.length, r),
                  })),
              ]);
              return (
                <text
