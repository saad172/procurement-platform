...
import type { loadSettlePage } from '@/db/queries/settle-page';
import { settleByHand } from '../actions';

/**
 * The Settle-row page's sections (SPEC §6.8) — one component per `<h2>`, plus
 * the chrome ahead of the first one.
 *
 * **The choice is the control.** What stood here asked a person to type a
 * 22-character opaque entity id into a free-text box, under a table that
 * repeated the same eight verdicts once per candidate. The three things that
 * changed are all one idea: **say what differs, once.** Verdicts every record
 * shares are stated above the list; records the checks cannot separate are
 * grouped; every remaining record is a radio, so the id never has to be seen.
 */
type Data = NonNullable<Awaited<ReturnType<typeof loadSettlePage>>>;

/**
 * A refusal is a reason not to proceed, which is what `.answer.stop` already
 * is — so it is one, rather than a fifth component wearing the same left
 * rule. The headline is constant because the outcome always is: every
 * refusal writes nothing, and only the reason varies.
 */
export function Answer({ data, programId }: { data: Data; programId: string }) {
  const { error, settled, answer } = data;
  if (error) {
    return (
...
/** ── What the eight checks already settled ── */
export function SharedChecks({ data }: { data: Data }) {
  const { shared } = data;
...
/** ── Which record settles it ── */
export function SettleForm({ data, programId }: { data: Data; programId: string }) {
  const { supplier, groups } = data;
  return (
    <>
...
export function PastAttempts({ data }: { data: Data }) {
  const { attempts, done } = data;
