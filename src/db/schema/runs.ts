 ...
 */
export const traceTurn = pgTable(
  'trace_turn',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    jobId: uuid('job_id')
 ...
 */
export const traceToolCall = pgTable(
  'trace_tool_call',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const usageEvent = pgTable(
  ...
  (t) => [index('usage_event_run_idx').on(t.runId), index('usage_event_job_idx').on(t.jobId)],
);
