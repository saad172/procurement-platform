 ...
 * nobody sourced.
 *
 * **`confirmed` is `false` on every row.** It stays false until a person has
 * read `docs/seed/expected-matches-draft.md` and said otherwise, and
...
export type ExpectedMatch = {
  ...
  reason: string;
  /** Always `false` until a person confirms it. See the header. */
  confirmed: boolean;
};
...
export const EXPECTED_MATCHES: readonly ExpectedMatch[] = [
  ...
  },
  {
    ...
    expected: { entityId: 'BAX_tQm9fXltkk3ZkhTAVA', label: 'Flex-n-Gate, LLC' },
    confidence: 'low',
    reason:
      'The one Candidate whose LEI GLEIF corroborates against the roster is 07zRxTFDUaxAHjHeDTCSWg — LEI 549300REAUV9ZD1VE488, which GLEIF names FLEX-N-GATE LLC in US-IL at URBANA, the roster’s own town — but Sayari labels that record FLEX N GATE COVINGTON, a plant. This record carries the parent’s legal name and sits at Troy MI. The label and the LEI point at different records, and neither is at 1306 East University Avenue.',
    confirmed: false,
  },
  {
    rosterIndex: 29,
