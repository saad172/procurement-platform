...
async function main() {
  ...
  const rows = ((row.body as { data?: unknown[] }).data ?? []) as { label: string; logisticsEntity?: boolean; metadata: { shipments: number } }[];
  ...
  console.log(`name heuristic caught ${tp}, missed ${fn}, false-flagged ${fp}, correctly ignored ${tn}`);
  ...
  for (const r of top.filter((r) => r.logisticsEntity)) {
    console.log(`    ${r.metadata.shipments.toString().padStart(6)}  ${r.label}  ${prefilterScore(r.label) < 0 ? '(name-caught)' : '(NAME MISSED)'}`);
  }
